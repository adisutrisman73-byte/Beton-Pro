...
// === INTERACTIVE LAYOUT SCHEMATICS ===

export interface FrameNode {
  id: string;
  gridX: number; // grid coordinate X (integer indices, e.g., 0, 1, 2...)
  gridY: number; // grid coordinate Y (integer indices)
  b: number; // column width, mm
  h: number; // column depth, mm
  fc: number; // concrete strength f'c, MPa
  fy: number; // rebar strength fy, MPa
  directLoad: number; // additional point load on column, kN
  height: number; // floor tributary height, m
  rebarCount: number; // e.g. 4, 6, 8
  rebarDiameter: number; // e.g. 16, 19, 22 mm
}
...
export interface FrameBeam {
  ...
  h: number; // height, mm
  fc: number; // concrete strength f'c, MPa
  fy: number; // rebar yield strength fy, MPa
  fys: number; // stirrups yield strength fys, MPa
  uniformLoad: number; // distributed uniform load q, kN/m
  rebarCount: number; // rebar count
  rebarDiameter: number; // rebar diameter, mm
  stirrupDiameter: number; // stirrup size, mm
  stirrupSpacing: number; // spacing, mm
}

export interface NodeAnalysisDetail {
  nodeId: string;
  PuTotal: number; // cumulative vertical load in column (direct + sum of tributary beam reactions), kN
  phiPn: number; // axial structural compressive capacity, kN
  DCR: number;
  isSafe: boolean;
}

export interface BeamAnalysisDetail {
  beamId: string;
  spanLength: number; // length in m
  Mu: number; // ultimate bending moment, kNm
  Vu: number; // ultimate shear force, kN
  phiMn: number; // ultimate moment capacity, kNm
  phiVn: number; // design shear capacity, kN
  DCR_bending: number;
  DCR_shear: number;
  isSafe: boolean;
...
export interface FrameLayoutAnalysis {
  nodes: Record<string, NodeAnalysisDetail>;
  beams: Record<string, BeamAnalysisDetail>;
  overallStatusSafe: boolean;
}
